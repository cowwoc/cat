...
EOF
    ...
    run bash "$CLAUDE_PLUGIN_ROOT/migrations/2.1.sh"
    [ "$status" -eq 0 ]
    run grep "^## Execution Waves" ".claude/cat/issues/v2/v2.1/test-issue/PLAN.md"
    [ "$status" -eq 0 ]
    run grep "^### Wave 1" ".claude/cat/issues/v2/v2.1/test-issue/PLAN.md"
    [ "$status" -eq 0 ]
    run grep "^## Execution Steps" ".claude/cat/issues/v2/v2.1/test-issue/PLAN.md"
    [ "$status" -ne 0 ]
}
...
EOF
    ...
    run bash "$CLAUDE_PLUGIN_ROOT/migrations/2.1.sh"
    [ "$status" -eq 0 ]
    plan_file=".claude/cat/issues/v2/v2.1/test-issue/PLAN.md"
