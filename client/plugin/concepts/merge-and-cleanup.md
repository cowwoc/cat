...
If no structured input tool is available and trust is not `high`, fail closed. Do not accept plain chat
approval.

Display this context before invoking the structured input tool:
```markdown
...
Only an explicit structured selection of `Approve and merge` approves the merge.
