...
|-----------|----------------------|---------|
| **Portable plugin hook files** | `plugin/hooks/common/` | Shared CAT plugin hook documentation/helpers |
| **Claude plugin hooks** | `plugin/hooks/claude/hooks.json` | CAT plugin behavior in Claude Code |
| **Codex plugin hooks** | `plugin/hooks/codex/hooks.json` | CAT plugin behavior in Codex |
| **Project hooks** | `.claude/settings.json` | Project-specific overrides |

### Plugin Hooks

Claude plugin hooks are registered in the plugin's Claude-specific hook config:

```
plugin/
├── hooks/
│   ├── common/           ← Shared hook documentation/helpers
│   ├── claude/
│   │   └── hooks.json    ← Claude plugin hook registration
│   └── codex/
│       └── hooks.json    ← Codex plugin hook registration
```
...
When investigating plugin hook behavior, check `plugin/hooks/claude/hooks.json` or
`plugin/hooks/codex/hooks.json` first, not `.claude/settings.json`. The project settings file is for
user-defined project hooks, not plugin-provided hooks.
