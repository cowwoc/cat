...
# 25. Rename .cat/rules/INDEX.md → .cat/rules/common/index.md
...
# 28. Move interim project rules from .agents/rules/ and root .cat/rules/*.md into .cat/rules/common/,
#     and move .codex/rules/ into .cat/rules/codex/
# 29. Install or update project-scoped Codex custom agent copies
...
log_migration "Phase 21: Rename caution config values: none→low, changed→medium, all→high"
...
# ──────────────────────────────────────────────────────────────────────────────
# Phase 25: Rename .cat/rules/INDEX.md → .cat/rules/common/index.md
# ──────────────────────────────────────────────────────────────────────────────
# Idempotent: only renames the file if it still has the uppercase name

log_migration "Phase 25: Rename .cat/rules/INDEX.md → .cat/rules/common/index.md"

if [[ -f ".cat/rules/INDEX.md" ]]; then
    mkdir -p ".cat/rules/common"
    if [[ -f ".cat/rules/common/index.md" ]]; then
        log_migration "  Phase 25: .cat/rules/common/index.md already exists - skipping"
    else
        mv ".cat/rules/INDEX.md" ".cat/rules/common/index.md"
        log_migration "  Renamed: .cat/rules/INDEX.md → .cat/rules/common/index.md"
    fi
else
...
# ──────────────────────────────────────────────────────────────────────────────
# Phase 28: Move interim project rules into .cat/rules/common/ and .cat/rules/codex/
# ──────────────────────────────────────────────────────────────────────────────
# Idempotent: moves remaining files only when the destination path does not already exist

log_migration "Phase 28: Move interim project rules into .cat/rules/common/ and .cat/rules/codex/"
...
move_project_rules() {
    ...
    if [[ ! -d "$source_dir" ]]; then
        log_migration "  Source not found: $source_dir"
        return 0
    fi
    ...
    while IFS= read -r rule_file; do
        ...
        local relative_rule_path="${rule_file#${source_dir}/}"
        local target_rule_file="${target_dir}/${relative_rule_path}"
        mkdir -p "$(dirname "$target_rule_file")"

        if [[ -e "$target_rule_file" ]]; then
            ((skipped++)) || true
            log_migration "  Skipped existing rule: $target_rule_file"
            continue
        fi

        mv "$rule_file" "$target_rule_file"
        ((moved++)) || true
        log_migration "  Moved: $rule_file → $target_rule_file"
    done <<< "$(find "$source_dir" -type f 2>/dev/null || true)"

    if [[ -z "$(find "$source_dir" -type f 2>/dev/null)" ]]; then
        rm -rf "$source_dir"
        log_migration "  Removed empty source directory: $source_dir"
    fi
    log_migration "  $source_dir complete: $moved files moved, $skipped skipped"
}

move_root_project_rules_to_common() {
    local source_dir=".cat/rules"
    local target_dir=".cat/rules/common"
    local moved=0
    local skipped=0
    ...
    while IFS= read -r rule_file; do
        ...
        if [[ -e "$target_rule_file" ]]; then
            ((skipped++)) || true
            log_migration "  Skipped existing root rule: $target_rule_file"
            continue
        fi

        mv "$rule_file" "$target_rule_file"
        ((moved++)) || true
        log_migration "  Moved root rule: $rule_file → $target_rule_file"
    done <<< "$(find "$source_dir" -maxdepth 1 -type f 2>/dev/null || true)"

    log_migration "  $source_dir root complete: $moved files moved, $skipped skipped"
}

move_project_rules ".agents/rules" ".cat/rules/common"
move_root_project_rules_to_common
move_project_rules ".codex/rules" ".cat/rules/codex"
log_migration "Phase 28 complete"

# ──────────────────────────────────────────────────────────────────────────────
# Phase 29: Install/update project-scoped Codex custom agent copies
# ──────────────────────────────────────────────────────────────────────────────
# Idempotent: only CAT-owned .codex/agents/cat-*.toml entries are managed
...
if [[ "$is_codex_runtime" == "true" ]]; then
    ...
    if [[ -d "$source_agents" ]]; then
        ...
        shopt -s nullglob
        for target_agent in "$target_agents"/cat-*.toml; do
            [[ -f "$target_agent" || -L "$target_agent" ]] || continue
            source_name="${target_agent##*/cat-}"
            source_agent="${source_agents}/${source_name}"

            if [[ ! -f "$source_agent" ]]; then
                rm -f "$target_agent"
                ((phase29_changed++)) || true
                log_migration "  Removed stale CAT Codex agent: $target_agent"
            fi
        done

        for source_agent in "$source_agents"/*.toml; do
            [[ -f "$source_agent" ]] || continue
            target_agent="${target_agents}/cat-${source_agent##*/}"

            if [[ -f "$target_agent" && ! -L "$target_agent" ]] && cmp -s "$source_agent" "$target_agent"; then
                continue
            fi

            rm -f "$target_agent"
            cp "$source_agent" "$target_agent"
            log_migration "  Copied Codex agent: $target_agent"
            ((phase29_changed++)) || true
        done
        shopt -u nullglob

        if [[ "$phase29_changed" -gt 0 ]]; then
            log_migration "Phase 29 complete: installed or updated $phase29_changed CAT Codex agent file(s)"
            log_migration "Restart, resume, or clear Codex for custom agent changes to take effect"
        else
            log_migration "Phase 29 complete: Codex custom agent copies already current"
        fi
    else
