...
**ALGORITHM DOCUMENTATION ONLY. Engine logic implemented in `util.WorkPrepare` Java class (launcher: `client/bin/work-prepare`).**
...
```json
{
  ...
  "worktree_path": "${HOME}/.cat/worktrees/2.1-issue-name",
...
RESULT=$("${CAT_PLUGIN_ROOT}/client/bin/work-prepare" --session-id "${SESSION_ID}" --exclude-pattern "compress*")

# When no filter or inclusion filter:
RESULT=$("${CAT_PLUGIN_ROOT}/client/bin/work-prepare" --session-id "${SESSION_ID}")
```
...
- Error with foreign session lock (`status: ERROR` + `locked_by` field present) - Check whether the `ERROR` response includes a `locked_by` field:
  - **`locked_by` present (foreign-session lock):** Check the `stale` field first:
    ...
    **If user selects Resume:**
    ```bash
    TRANSFER_RESULT=$("${CAT_PLUGIN_ROOT}/client/bin/issue-lock" transfer "${ISSUE_ID}" "${locked_by}" "${CAT_SESSION_ID}" "${worktree_path}")
...
`"${CAT_PLUGIN_ROOT}/client/bin/issue-lock" release "${issue_id}" "${CAT_SESSION_ID}"` and
...
1. **Check if the worktree path is a valid, accessible directory** (`[ -d "$WORKTREE_PATH" ]` returns
   ...
   `git -C "${CAT_PROJECT_DIR}" worktree prune` and proceed with normal issue discovery. Do NOT
...
2. **Check merge status first.** Determine whether the branch's commits are already merged into base:
   ...
   UNMERGED=$(git -C "${CAT_PROJECT_DIR}" log --oneline "${TARGET_BRANCH}..${ISSUE_BRANCH}" 2>/dev/null)
   ```
...
3. **If already merged** (`UNMERGED` is empty): The work was completed in a prior session and merged. Clean up:
   ...
   git -C "${CAT_PROJECT_DIR}" worktree remove "${WORKTREE_PATH}" --force
   git -C "${CAT_PROJECT_DIR}" branch -D "${ISSUE_BRANCH}" 2>/dev/null || true
   ```
...
4. **If NOT merged** (`UNMERGED` has commits): Prior work exists but was not merged. Resume work in the existing
   worktree:
   - Acquire the lock: run `"${CAT_PLUGIN_ROOT}/client/bin/issue-lock" acquire "${issue_id}" "${CAT_SESSION_ID}" "${WORKTREE_PATH}"`
...
```bash
PLAN_MD="${CAT_PROJECT_DIR}/.cat/issues/v${MAJOR}/v${MAJOR}.${MINOR}/${ISSUE_NAME}/plan.md"
```
...
WORKTREE_PATH="${HOME}/.cat/worktrees/${ISSUE_BRANCH}"
...
```bash
EXISTING_WORK_RESULT=$("${CAT_PLUGIN_ROOT}/client/bin/check-existing-work" \
  --worktree "${WORKTREE_PATH}" \
...
ISSUE_COMMITS=$(git -C "${CAT_PROJECT_DIR}" log --oneline \
...
git -C "${CAT_PROJECT_DIR}" log --oneline "${TARGET_BRANCH}" -20 2>/dev/null
GIT_EXIT=$?
if [[ $GIT_EXIT -ne 0 ]]; then
  echo "WARNING: git log failed (exit ${GIT_EXIT}) for Strategy 2 file-overlap search." >&2
  STRATEGY2_FAILED=true
else
  git -C "${CAT_PROJECT_DIR}" log --oneline "${TARGET_BRANCH}" -20 | while read -r hash msg; do
    git -C "${CAT_PROJECT_DIR}" diff-tree --no-commit-id -r --name-only "$hash" | \
...
# INDEX_FILE="${CAT_PROJECT_DIR}/.cat/issues/..."
```
...
discards all existing fields (dependencies, blocks, target_branch, etc.).
...
**Do NOT modify any files in `${CAT_PROJECT_DIR}` directly.** All file modifications
