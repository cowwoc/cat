...
- See `${CAT_PLUGIN_ROOT}/templates/issue-index.json` for the issue index.json template.
- See `${CAT_PLUGIN_ROOT}/templates/issue-plan.md` for issue plan.md templates.
- See `${CAT_PLUGIN_ROOT}/templates/major-state.md` and `${CAT_PLUGIN_ROOT}/templates/major-plan.md` for major version templates.
- See `${CAT_PLUGIN_ROOT}/templates/minor-state.md` and `${CAT_PLUGIN_ROOT}/templates/minor-plan.md` for minor version templates.
- See `${CAT_PLUGIN_ROOT}/templates/changelog.md` for the changelog.md template.
- See `${CAT_PLUGIN_ROOT}/concepts/questioning.md` for smart questioning guidance.
- See `${CAT_PLUGIN_ROOT}/concepts/version-paths.md` for version path conventions.
...
- Key skill dependencies by scanning for patterns like `client/plugin/skills/{common,<engine>}/<name>/` or "modify <name> skill"
...
**Render proposal display box via cat:get-output:**
...
Construct the `indexContent` field as valid JSON matching `${CAT_PLUGIN_ROOT}/templates/issue-index.json`:
...
later by `cat:work-implement` when work begins.
...
**JSON escaping:** All interpolated string values ({issue-name}, {one-line description}, dependency names,
dependent index file paths) must be valid JSON string values. Escape any double quotes as `\"` and backslashes as
...
"${CAT_PLUGIN_ROOT}/client/bin/create-issue" --json '{
  ...
  "dependent_index_files": [${AUTO_DETECTED_DEP_PATHS_JSON}],
...
`issue_detect_skill_deps`. This is done atomically by `create-issue` to ensure dependency updates are committed
with issue creation and never orphaned.

If AUTO_DETECTED_DEP_PATHS is non-empty, pass it through `dependent_index_files` in the create-issue JSON payload.
`create-issue` adds `{new-issue-id}` to each target index.json `dependencies` list idempotently.
No post-create shell mutation or extra commit is required in the skill.
...
```bash
CLIENT_BIN=$(find "${CAT_PROJECT_DIR}/client" -path "*/target/jlink/${CAT_ENGINE:-claude}/bin" -type d -print -quit)
...
**Create major plan.md** using the template at `${CAT_PLUGIN_ROOT}/templates/major-plan.md`:
