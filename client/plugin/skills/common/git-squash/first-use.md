...
**Cross-type example (same topic, different prefixes):** Commits `bugfix: fix skill-loading argument parsing` and
`refactor: change skill-loading argument handling to accept pre-tokenized inputs` both address the same
concern. Squash to one: `bugfix: fix skill-loading argument parsing and pre-tokenize inputs`
...
```bash
"${CAT_PLUGIN_DATA}/client/bin/git-squash" "<target-branch>" "$MESSAGE" "$WORKTREE_PATH"
```
