...
```bash
"${CAT_PLUGIN_ROOT}/client/bin/get-config-output" effective
```
...
```bash
"${CAT_PLUGIN_ROOT}/client/bin/update-config" "fileWidth={value}"
```
...
```bash
"${CAT_PLUGIN_ROOT}/client/bin/update-config" "completionWorkflow={value}"
```
...
```bash
"${CAT_PLUGIN_ROOT}/client/bin/update-config" "minSeverity={value}"
```
