...
# RECOMPUTED_HASH=$(printf "%s:%s" "$CAT_SESSION_ID" "$STORED_EPOCH" | sha256sum | cut -d' ' -f1)
...
>
> For each phase:
> 1. Use the Read tool to load the phase file from ${CAT_PLUGIN_ROOT}/skills/learn/
...
>
> **Phase files to load:**
> - Phase 1 (Investigate): ${CAT_PLUGIN_ROOT}/skills/learn/phase-investigate.md
> - Phase 2 (Analyze): ${CAT_PLUGIN_ROOT}/skills/learn/phase-analyze.md
> - Phase 3 (Prevent): ${CAT_PLUGIN_ROOT}/skills/learn/phase-prevent.md
>
...
1. **Validate `issue_creation_info` before proceeding (MANDATORY enforcement).** Verify that:
   ...
   Please ask the agent to add the issue manually with the following values:
...
3. Invoke `/cat:add {suggested_title}` where `{suggested_title}` is the one-line summary from
   ...
   Ask the agent to add the issue manually with the following values:
...
options:
  ...
  - label: "Later"
    action: "Inform user to ask the agent to run a retrospective when ready"
