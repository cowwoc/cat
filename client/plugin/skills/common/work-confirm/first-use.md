...
```
<issue_id> <issue_path> <worktree_path> <issue_branch> <target_branch> <execution_commits_json_path> <files_changed> <trust> <caution>
```
...
| 5 | execution_commits_json_path | Path to JSON file containing commit objects from the implement phase |
| 6 | files_changed | integer count of files changed |
| 7 | trust | `medium` |
| 8 | caution | `medium` |
...
```bash
read ISSUE_ID ISSUE_PATH WORKTREE_PATH BRANCH TARGET_BRANCH EXECUTION_COMMITS_JSON_PATH FILES_CHANGED TRUST CAUTION <<< "$ARGUMENTS" && \
PLAN_MD="${ISSUE_PATH}/plan.md" && \
"${CAT_PLUGIN_DATA}/client/bin/progress-banner" ${ISSUE_ID} --phase confirming
```
