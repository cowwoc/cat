...
**Invoke `cat:git-squash` before invoking this skill.** Squashing after the merge is not possible.
...
(not just reduce the count by one). Squash by topic first using `cat:git-squash`, then retry.

## Prerequisites

- [ ] `cat:git-squash` invoked and squash completed (see Squash-by-Topic First above)
...
```bash
"${CAT_PLUGIN_ROOT}/client/bin/git-merge-linear" \
