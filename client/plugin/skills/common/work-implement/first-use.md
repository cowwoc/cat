...
| 1 | issue_path | `${WORKTREE_PATH}/.cat/issues/v2/v2.1/issue-name` |
| 2 | worktree_path | `${HOME}/.cat/worktrees/2.1-issue-name` |
| 3 | issue_branch | `2.1-issue-name` |
| 4 | target_branch | `v2.1` |
| 5 | estimated_tokens | `45000` |
| 6 | trust | `medium` |
| 7 | caution | `changed` |
...
```bash
if [[ -z "${CAT_SESSION_ID:-}" ]]; then
  echo "ERROR: CAT_SESSION_ID environment variable is not set"
  exit 1
fi
...
if [[ "${LOCK_SESSION}" == "${CAT_SESSION_ID}" ]]; then
  echo "OK: Lock verified for current session"
else
  echo "ERROR: Lock for ${ISSUE_ID} belongs to session ${LOCK_SESSION}, not ${CAT_SESSION_ID}"
  exit 1
fi
```
...
Skill tool:
  ...
  args: "${CURIOSITY} revise ${ISSUE_PATH} Generate full implementation steps for this lightweight
plan. Add Jobs or Execution Steps section with detailed step-by-step implementation guidance."
```

3. After plan-builder returns, commit the updated plan.md:
...
**If `hasSteps=true`** (full plan with implementation steps): skip plan-builder invocation. Proceed directly
...
Then invoke:
`Skill("cat:plan-builder", "${CURIOSITY} revise ${ISSUE_PATH} <description of what changed>")`
...
Skill tool:
  ...
  args: "${SUBAGENT_RAW_ID} ${ISSUE_PATH} <subagentCommitsJsonPath>"
```
...
**Parallel job completion protocol — wait for ALL jobs before calling collect-results for ANY:**
...
6. Call collect-results for job `NEXT_COLLECT`. Before each call, verify `NEXT_COLLECT` equals
...
`cat:git-amend`. Do NOT use `git commit --amend` directly.
...
**For multiple mismatched commits:** Use `cat:git-amend` for each commit that needs correction,
...
- `git commit --amend` (use `cat:git-amend` instead)
- `git rebase -i` (prohibited by project instructions; interactive rebase requires the `-i` flag which is blocked)
