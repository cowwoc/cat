...
for pattern in "${PATTERNS_TO_REMOVE[@]}"; do
  ...
  grep -n "$pattern" /path/to/hooks/example-hook.sh || echo "  No matches"
done
```
...
for file in /path/to/hooks/*.sh; do
...
ls -la /path/to/hooks/*.backup-* | head -5
```

**Verify Backups Created**:
```bash
# Count backups
BACKUP_COUNT=$(find /path/to/hooks -name "*.backup-*" | wc -l)
ORIGINAL_COUNT=$(find /path/to/hooks -name "*.sh" ! -name "*.backup-*" | wc -l)
...
# Run with pattern
/tmp/safe-pattern-removal.sh "PATTERN_TO_REMOVE" "/path/to/hooks" 10
```
...
for hook in /path/to/hooks/*.sh; do
...
if [[ -f /path/to/hooks/tests/test-hooks.sh ]]; then
  bash /path/to/hooks/tests/test-hooks.sh || {
...
SAMPLE_FILES=(
  "/path/to/hooks/auto-learn.sh"
  "/path/to/hooks/enforce-commit-squashing.sh"
  "/path/to/hooks/load-todo.sh"
)
...
for backup in /path/to/hooks/*.backup-*; do
...
for pattern in "${PATTERNS[@]}"; do
  echo "  Pattern: $pattern"
  grep -c "$pattern" /path/to/hooks/*.sh 2>/dev/null | grep -v ":0" || echo "    No matches"
done
echo ""
...
for pattern in "${PATTERNS[@]}"; do
  /tmp/safe-pattern-removal.sh "$pattern" "/path/to/hooks" 10
done
...
echo "Sample files:"
head -30 /path/to/hooks/enforce-checkpoints.sh
echo ""
