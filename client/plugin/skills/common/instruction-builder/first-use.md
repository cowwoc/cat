...
- Creating a new instruction document (skill, command, project instructions, rules file, etc.)
...
```bash
CURIOSITY=$("${CAT_PLUGIN_DATA}/client/bin/get-config-output" effective \
...
|---------|------------|---------|
| **File references** | `[description](${CAT_PLUGIN_ROOT}/path/file.md)` inside | Reference files for on-demand loading via Read tool |
|                     | `<execution_context>` |  |
...
Skill tests are markdown files in the source checkout under `client/plugin/tests/skills/<skill-name>/first-use/` that define scenarios and
...
Task tool:
  ...
  prompt: |
    ...
    ## Design Methodology
    Read and follow: ${CAT_PLUGIN_ROOT}/skills/instruction-builder/design-methodology.md

    ## Skill Writing Conventions
    Read and follow: ${CAT_PLUGIN_ROOT}/skills/instruction-builder/skill-conventions.md
...
DESCRIPTION=$(printf '%s' "${INSTRUCTION_DRAFT}" | \
  "${CAT_PLUGIN_DATA}/client/bin/update-skill-description") || DESC_TOO_LONG=true
...
INSTRUCTION_DRAFT=$(printf '%s' "${INSTRUCTION_DRAFT}" | \
  "${CAT_PLUGIN_DATA}/client/bin/update-skill-description" "${NEW_DESCRIPTION}")
```
...
```bash
TEST_DIR=$("${CAT_PLUGIN_DATA}/client/bin/instruction-test-runner" extract-test-dir \
...
```bash
TEST_MODEL=$("${CAT_PLUGIN_DATA}/client/bin/instruction-test-runner" extract-model \
...
- **Correct (production-sequence format):** `"I've finished implementing 2.1-my-issue and the commits
  ...
  `cat:git-squash` before the approval gate.
...
- **Correct (action-based assertion):** `"The Skill tool was invoked with skill cat:git-squash."` —
...
No blank lines between the closing `---` of frontmatter and `## Turn 1`. No license header (all files under `client/plugin/tests/` are exempt).

- **Prohibited (Q&A format for Turn 1):**
  ```
  ## Turn 1
  Is the prohibition against calling cat:git-squash before rebasing well-formed?
  ```
...
Commit all generated `.md` files with message `test: generate test cases [session: ${CAT_SESSION_ID}]`:
```bash
...
`client/plugin/skills/common/foo-agent/first-use.md` must reference only paths under `client/plugin/skills/common/foo-agent/` and
`client/plugin/tests/skills/common/foo-agent/`.
...
```bash
"${CAT_PLUGIN_DATA}/client/bin/instruction-test-runner" detect-changes \
...
Invoke `cat:sprt-runner` to run the full SPRT loop:

```
skill: "cat:sprt-runner"
args: "${TEST_DIR} ${WORKTREE_PATH} ${TEST_MODEL} ${TEST_EFFORT} ${INSTRUCTION_DRAFT_SHA}"
```
...
The token summary and per-test-case decisions are displayed by `cat:sprt-runner` as part of
...
**CRITICAL:** You MUST invoke cat:get-history for each scenario-provided ID, even if you believe the ID is
...
**Sub-step 3 — Retrieve full transcripts via cat:get-history** (automatic): Invoke
`cat:get-history` with the current `AGENT_ID`:
...
→ Next step: Proceed to Step 9 (cat:instruction-analyzer) to analyze the defect pattern.
```

**MANDATORY:** When conclusion is Genuine skill defect or Inconclusive, the report MUST include the
exact line "→ Next step: Proceed to Step 9 (cat:instruction-analyzer) to analyze the defect
...
**RESTRICTION — never modify tests to match the skill:** When a test case fails, you MUST fix the
skill instruction to make the test pass — **never change the test to match the skill's actual (broken)
behavior.** A test that exercises a valid business requirement is the ground truth; the skill is wrong,
not the test. Modifying a test to accommodate skill behavior that contradicts the requirement is a
workflow violation.

The only legitimate reason to change a test case is when the test itself is defective: a Q&A-format
prompt, a non-production-sequence turn, an assertion that is demonstrably never falsifiable, or a
test that targets the wrong requirement. If the investigation concludes "Genuine skill defect" and the
test content is objective and valid, the skill MUST be updated — period.

**RESTRICTION — append_system_prompt is off-limits:** When fixing a compliance failure, you MUST revise the
instruction file itself (the rule text, procedure wording, or constraint language). You MAY NOT work
around the failure by adding or modifying the `append_system_prompt` field in any test's `config.json`.
That field is reserved for test-scenario setup and is NOT a substitute for clear instruction text.

**Compliance escalation strategy** — if instruction revision alone is not achieving compliance after 2+
iterations, escalate in this order before adding a blocking hook:

1. **Instruction revision** (always first): improve clarity, add examples, strengthen wording.
2. **`additionalContext` injection** (second resort): add a `UserPromptSubmit` hook that detects the
   failing scenario and injects a short MANDATORY reminder pointing to the rule/skill file. This fires
   before the agent processes the user's message, reinforcing compliance at request time. Constraints:
   - The message may describe WHAT was detected that the agent might otherwise miss — information the
     hook inferred from the prompt that is NOT already stated there. Example: "DO NOT run `cmd | filter`
     as a bare pipe" names the specific problematic command the hook found.
   - The message must NOT repeat information already in the user's prompt (e.g., do not echo "the user
     wants background execution" — the agent already has the original prompt).
   - The message must NOT inline rule content: no commands, parameters, or implementation details that
     are already in the rule file. The rule file is authoritative — if a detail belongs there, it stays
     there. Example formats:
   ```
   MANDATORY: Apply plugin/rules/common/<rule-file>.md for this request.
   MANDATORY: DO NOT run `<detected-command>` as a bare pipe. Apply plugin/rules/common/<rule-file>.md.
   ```
   If the rule file references another file for a specific detail (e.g., a centralized mktemp guide),
   the hook message still only needs to reference the top-level rule file — following references is the
   agent's job.
3. **Blocking hook** (last resort): only add a `PreToolUse` blocking hook if both instruction revision
...
`test: restore best iteration [session: ${CAT_SESSION_ID}]`, then report "test plateau reached."
...
1. **Assertion-first hardening input:** When Step 6 returns any failed test case, extract failing assertion text from
   `${WORKTREE_ROOT}/.cat/work/test-runs/<session-id>/*_grade.json` and pass those assertion failures verbatim
   to red-team and blue-team prompts as required patch targets. Do not run loophole discovery in isolation from
   failing assertions.
2. **Failure strategy ledger:** Persist a ledger at `${WORKTREE_ROOT}/instruction-builder/hardening-ledger.json`.
...
4. **Targeted rerun gate before full SPRT:** After each blue-team patch, rerun previously-failed test cases first.
   Only run full SPRT after targeted rerun shows no failures for those cases.
5. **Strict diff-validation gate:** Diff-validation input must include pinned red-team commit hash and FINDINGS_PATH.
   If diff-validation reports zero processed findings while failures remain, treat the round as invalid and stop for
   re-run with corrected inputs.
6. **Non-convergence escalation:** If the same failure signature persists after two distinct strategies, stop normal
   loop iteration and run explicit test-rubric/investigation review before further hardening edits.
7. **Prompt-shape escalation for repeated obedience failures:** If repeated rounds still show the model executing
   prohibited command shapes despite policy text coverage, escalate the red-team/blue-team prompt contract itself
   (for example: add explicit forbidden/required first-command shapes tied to the failed assertion signatures,
   require first-command compliance checks in findings, and reject patches that only restate existing policy).
   Why: repeated failures in this state indicate patch-generation drift, so stronger patch-selection constraints are
   required in the adversarial loop rather than more paraphrased instruction text alone.
8. **Priority-trace diagnostic step (permanent, non-default):** For persistent failures after escalation, add
   temporary INVESTIGATION scenarios that require the runner to enumerate candidate approaches, rank them, and justify
   priority before execution, then compare declared priority against actual first command/tool behavior. Use this to
   detect rule-priority conflicts. Remove or disable these diagnostics once conflict hypotheses are validated.
...
2. Run the full RED→BLUE loop as defined in Step 10 and the shared adversarial protocol. Each round
   ...
   continues until convergence (red-team returns `has_critical_high: false`). Apply convergence controls:
   (a) assertion-first prompt inputs from failing grade artifacts, (b) hardening-ledger strategy dedup,
   (c) infrastructure-lane closure before policy edits, (d) targeted rerun of prior failing cases before
   full SPRT, and (e) strict diff-validation non-empty processed-findings gate.
...
Commit test results with message `test: post-hardening SPRT [session: ${CAT_SESSION_ID}]`.
...
Task tool:
  ...
  prompt: |
    ...
    Read and follow: ${CAT_PLUGIN_ROOT}/skills/instruction-builder/compression-protocol.md
    ...
    (3) ${CAT_PLUGIN_ROOT}/skills/instruction-builder/compression-protocol.md.
...
Commit the compressed file with message `refactor: compress instruction [session: ${CAT_SESSION_ID}]`.
...
`${CAT_PLUGIN_ROOT}/skills/instruction-builder/validation-protocol.md` (Section 2):
...
`test: mark protected sections [session: ${CAT_SESSION_ID}]`.
...
`refactor: accept final compression [session: ${CAT_SESSION_ID}]`. Do NOT commit the post-compression
...
**Scope**: Only files in the same skill directory (e.g., `client/plugin/skills/common/my-skill/`). Do not cross skill
boundaries.
...
- **Behavioral test cases**: SPRT calibration test cases for this skill are stored in
  `client/plugin/tests/skills/common/instruction-builder-agent/first-use/` (one `.md` file per test case). Test results
  are stored in `client/plugin/tests/skills/common/instruction-builder-agent/first-use/test-results.json`
...
- [ ] instruction-analyzer-agent spawned correctly with `TEST_SHA`, `TEST_PATH`, `INSTRUCTION_TEXT_PATH`,
  `WORKTREE_ROOT`, `TEST_DIR`, and `CAT_SESSION_ID`
