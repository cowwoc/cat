...
The description is used for **intent routing** — it tells the agent WHEN to invoke this skill based
...
- [ ] `argument-hint` documents only the arguments required by the called command
- [ ] Do not add placeholder arguments that are not consumed by the called command
...
| **Positive alternative** | Concrete next step the agent should take instead | "Use `/cat:rm` instead, which backs up before deleting." |
...
When a rule requires the agent to reproduce content verbatim (copy, echo, relay without modification):
...
reliability. Agents often treat XML-tagged content as structured data rather than prose, reducing the chance
...
markdown gets read by the agent, who then generates nearly identical bash via tool calls rather than
...
All deterministic bash belongs in external script files (`client/plugin/scripts/`). Skills contain only:
...
Scripts (bash in `client/plugin/scripts/`) handle:
...
- Operations have no judgment decisions (no branching based on the agent's analysis)
- The agent reads bash from markdown then generates nearly identical bash via tool calls

#### When NOT to Extract

Keep in skill markdown when:
- Operations require the agent's judgment (conflict resolution, commit message construction from topic analysis)
- Interactive workflows need user input mid-stream
- One-off operations that won't be repeated

#### Script Conventions

- Location: `client/plugin/scripts/<skill-name>.sh` or `client/plugin/scripts/<operation>.sh`
...
- Invocation from skill: `"$(git rev-parse --show-toplevel)/client/plugin/scripts/<name>.sh" "$ARG1" "$ARG2"`
...
(algorithms, formulas, calculations), the output MUST be generated BEFORE the agent sees the content.
CAT skill loading provides a built-in mechanism for this: **silent preprocessing**.
...
**patterns as examples**. The skill preprocessor scans the entire skill file and will attempt
...
The agent receives the rendered banner and outputs it directly.
...
1. Create script in `client/plugin/scripts/` (e.g., `cat-progress-banner.sh`)
...
For each function:
  ...
  Could agent compute wrong? ─No─→ Keep in skill (trivial/reliable)
        ...
        Yes
         │
  Extract to preprocessing script (`client/plugin/scripts/`)
         │
  Script computes output BEFORE the agent sees skill
         │
  The agent receives rendered output, outputs directly
```
...
| Changes | Different output each call | Same instructions, reused via first-use loading reference path |
...
- **Session-efficient**: The full skill content loads only once per session (via `${CAT_PLUGIN_ROOT}/rules/common/skill-loading.md`).
...
| Special handling for edge cases | "For project instructions files specifically..." | Extract to `PROJECT-INSTRUCTIONS-HANDLING.md` |
...
```
client/plugin/skills/common/my-skill/
...
**Shared files** (referenced by multiple skills/commands) belong in `client/plugin/concepts/`:
...
|---------------|----------|
| Single portable skill only | `client/plugin/skills/common/{skill-name}/` |
| Multiple skills | `client/plugin/concepts/` |

**After creating a concept file, wire it into agent context.** Files in `client/plugin/concepts/` are NOT loaded
...
2. Add a `[description](${CAT_PLUGIN_ROOT}/concepts/<filename>.md)` markdown link or
   `` `${CAT_PLUGIN_ROOT}/concepts/<filename>.md` `` inline code reference inside the skill's
   `first-use.md`.
...
If no existing skill is the right home, add the concept to a rules file in `.cat/rules/common/` instead —
rules files ARE injected automatically into the main agent's context.
