...
the CAT skill loader's **silent preprocessing** feature with the [BANG]`command` syntax.
...
**Pattern:**
```
Skill loads → Commands execute silently → Agent receives rendered content → Output verbatim
```
...
- [ ] The agent outputs the result exactly as received
...
[Agent receives already-rendered content]
