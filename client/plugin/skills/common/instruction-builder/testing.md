...
## Command Cost in Test Scenarios

**Use the cheapest command that still tests the target behavior.**

SPRT runs many trials in parallel — each trial spawns an agent that executes the scenario's command in
an isolated runner worktree. Expensive commands (build tools, test suites) multiply: 10 parallel trials ×
8 SPRT instances = dozens of concurrent processes that can exhaust CPU, memory, and I/O.

**Prefer instant commands over build commands:**

| Instead of | Use |
|---|---|
| `mvn test` | `ps aux`, `git log`, `find`, `ls` |
| `mvn compile` | `find . -name "*.java" \| head` |
| `mvn verify` | `git log --oneline \| grep -i fix` |
| `npm test` | `find . -name "*.ts" \| wc -l` |

The rule or skill being tested almost never requires the real build to run. A scenario testing tee-piped-output
behavior needs only *a command that produces output through a pipe* — `ps aux | grep java` tests the same
compliance as `mvn compile | grep error`, but runs in milliseconds.

**Design principle:** Ask "what is the minimum computation that makes the agent face this decision?" Use that.
