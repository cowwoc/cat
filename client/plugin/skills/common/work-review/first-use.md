...
```
<issue_id> <issue_path> <worktree_path> <issue_branch> <target_branch> <all_commits_compact> <trust> <caution>
```
...
| 5 | all_commits_compact | compact format `hash:type,hash:type` |
| 6 | trust | `medium` |
| 7 | caution | `medium` |
...
```bash
read ISSUE_ID ISSUE_PATH WORKTREE_PATH BRANCH TARGET_BRANCH ALL_COMMITS_COMPACT TRUST CAUTION <<< "$ARGUMENTS" && \
PLAN_MD="${ISSUE_PATH}/plan.md" && \
"${CAT_PLUGIN_ROOT}/client/bin/progress-banner" ${ISSUE_ID} --phase reviewing
```
...
```bash
CONFIG_FILE="${CAT_PROJECT_DIR}/.cat/config.json"
...
3. Spawn a shared planning agent to produce a per-concern fix plan:
   ...
   **Repeated UNFIXABLE escalation (MANDATORY):** Track UNFIXABLE declarations per concern using the match key
   ...
   - "Defer to tracking issue": move to `DEFERRED_CONCERNS` and create a tracking issue via `/cat:add`
...
8. **Fallback for failed concern fix agents:**
    ...
    - "Skip this concern": move concern N to `DEFERRED_CONCERNS`. If severity is CRITICAL or HIGH, create a
      tracking issue via `/cat:add` per the severity × decision matrix.
...
| Severity | Perfection | Target Version | `/cat:add` args prefix |
|----------|----------|----------------|------------------------|
...
`work-with-issue` orchestrator that the review phase is complete and it must proceed to Phase 4: Merge.
...
immediately invokes `cat:work-merge` — no user confirmation or additional summary is needed here.
