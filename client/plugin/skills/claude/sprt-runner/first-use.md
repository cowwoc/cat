...
The `instruction-test-runner` binary exposes related subcommands for running SPRT:

| Command | Scope | Use case |
|---------|-------|----------|
| `run-sprt` | Complete SPRT workflow (public entry point) | Invoked by `sprt-runner-agent` to run a full suite or filtered subset (via `<pattern>`). Orchestrates prepare, isolation, batching, grading, state updates, results, and cleanup. |
| `run-sprt-batch` | Single SPRT batch (internal workhorse) | Called internally by `run-sprt` loop to process one batch of trials. Creates runner worktrees, launches parallel trials, grades outputs, and updates SPRT state. |

`run-sprt` usage:

```bash
instruction-test-runner run-sprt \
  <worktree_path> <test_dir> <test_model> <session_id> [<effort>]
```
...
plugin version — neither depends on `CAT_PLUGIN_ROOT`.
...
The SPRT workflow is executed via `instruction-test-runner run-sprt` with the current parameter contract (7 positional arguments after optional flags).
...
5. If a session exists, invoke the `cat:get-history` skill on it:
   - Use `session-analyzer --runtime claude errors <session_id>` to surface tool errors
   - Use `session-analyzer --runtime claude search <session_id> "keyword"` to find relevant events
