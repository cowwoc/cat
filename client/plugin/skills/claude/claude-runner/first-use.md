...
| `--model <name>` | yes | Model short name: `haiku`, `sonnet`, or `opus` |
| `--effort <level>` | yes | Reasoning effort: `low`, `medium`, `high`, `xhigh`, or `max` |
| `--cwd <path>` | no | Working directory for the nested instance. If omitted, uses the current directory of the `claude-runner` process. |
| `--plugin-source <path>` | no | Plugin source directory to copy into an isolated Claude config cache. If omitted, no isolated plugin cache is created unless `--jlink-bin` is also provided. |
| `--jlink-bin <path>` | no | jlink binary directory to copy into an isolated Claude config cache. If omitted, no isolated plugin cache is created unless `--plugin-source` is also provided. |
| `--plugin-version <ver>` | no | Plugin version string used only when creating an isolated plugin cache. If omitted, uses `2.1`. |
| `--agent <name>` | no | Launch nested instance with `claude --agent <name>`. If omitted, runs as a normal nested main agent. |
| `--append-system-prompt <text>` | no | Append text to the system prompt of the nested instance. If omitted, no extra system prompt is appended. |
| `--output <path>` | no | Write JSON results to file. If omitted, parsed JSON is not written to disk; text output is still printed to stdout. |

## Omitted Optional Arguments

- If `--cwd` is omitted, the nested Claude process runs from the current working directory of `claude-runner`.
- If both `--plugin-source` and `--jlink-bin` are omitted, `claude-runner` does not create an isolated Claude config or refresh the plugin cache.
- If exactly one of `--plugin-source` or `--jlink-bin` is omitted, no isolated plugin cache is created; pass both when testing updated plugin code.
- If `--plugin-version` is omitted, the isolated cache path uses version `2.1`.
- If `--agent` is omitted, the nested process runs as a regular Claude main agent, not a subagent wrapper.
- If `--append-system-prompt` is omitted, the nested process receives only Claude's normal system prompt plus the prompt file content.
- If `--output` is omitted, the runner prints text output to stdout but does not persist the parsed JSON result.
...
The `--effort` argument is required so nested runs do not silently inherit or guess a reasoning level.
...
- [ ] All paths passed to the runner are validated:
  ...
  - `--plugin-source` is `$PLUGIN_SOURCE` (worktree's `client/plugin/` directory)
