...
"$SESSION_ANALYZER" --engine codex search "${CODEX_THREAD_ID}" "keyword" --context 2
"$SESSION_ANALYZER" --engine codex errors "${CODEX_THREAD_ID}"
"$SESSION_ANALYZER" --engine codex file-history "${CODEX_THREAD_ID}" "config.json"
"$SESSION_ANALYZER" --engine codex analyze "${CODEX_THREAD_ID}"
```

Use `--codex-home <path>` to inspect an isolated Codex home:

```bash
"$SESSION_ANALYZER" --engine codex --codex-home "$ISOLATION_CODEX_HOME" analyze "$SESSION_ID"
```
...
"$SESSION_ANALYZER" --engine codex analyze "$SUBAGENT_THREAD_ID"
"$SESSION_ANALYZER" --engine codex search "$SUBAGENT_THREAD_ID" "instruction-builder" --context 2
```
...
"$SESSION_ANALYZER" --engine codex analyze "$SUBAGENT_TRANSCRIPT"
```
