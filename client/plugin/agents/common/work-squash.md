...
- Use `"${CAT_PLUGIN_ROOT}/client/bin/git-squash"` for commit squashing (never `git rebase -i`)
...
AUTO_CLOSE_OUTPUT=$("${CAT_PLUGIN_ROOT}/client/bin/auto-close-index" "${WORKTREE_PATH}" "${BRANCH}")
...
```bash
"${CAT_PLUGIN_ROOT}/client/bin/git-squash" "${TARGET_BRANCH}" "${PRIMARY_COMMIT_MESSAGE}" "${WORKTREE_PATH}"
SQUASH_EXIT=$?
```
