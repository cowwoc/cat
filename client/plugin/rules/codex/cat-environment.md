...
running a Bash command that references CAT paths, initialize CAT's engine-neutral variables explicitly.

Use this block at the top of Bash commands that need `CAT_PLUGIN_ROOT`, `CAT_PLUGIN_DATA`,
`CAT_PROJECT_DIR`, `CAT_ENGINE`, or `CAT_SESSION_ID`:
...
# If CODEX_THREAD_ID is unavailable, stop and report the missing engine context.
...
Use `CAT_PLUGIN_DATA` for mutable engine data generated outside the installed plugin.
