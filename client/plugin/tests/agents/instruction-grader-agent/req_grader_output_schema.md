...
1. **Transcript**: The agent wrote a skill instruction file with content "Shows the current date",
   then invoked the Skill tool with skill `cat:instruction-builder`. The skill failed because
   the CAT client binary could not be downloaded.
2. **Scenario file path**: client/plugin/tests/agents/instruction-grader-agent/fixtures/grader-schema-scenario.md
...
3. The first entry's `assertion` field is exactly "The Skill tool was invoked with skill instruction-builder"
