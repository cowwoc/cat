...
agent. The nested prompt should tell Claude to use the Agent tool to spawn an agent that
replies with exactly `SUBAGENT_SPAWNED_OK_c4e8f1a3`, then report the agent response. Capture
...
4. the nested prompt instructs Claude to use the Agent tool to spawn an agent
