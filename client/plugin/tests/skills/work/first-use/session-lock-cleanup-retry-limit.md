...
The user selects "Clean up and retry". Cleanup completes, then the immediate retry returns the same existing-session-lock error again.
...
5. When the retry returns the same existing-session-lock error, the agent displays the error verbatim and stops.
6. The agent does not loop back to the user-choice prompt after the second existing-session-lock error.
