...
BUILD_SCRIPT="${SCRIPT_DIR}/distribution/scripts/build-runtime-artifacts.sh"
...
setup() {
    ...
    cp "${BUILD_SCRIPT}" "${DISTRIBUTION_DIR}/scripts/build-runtime-artifacts.sh"
...
write_builder_stub() {
    cat > "${DISTRIBUTION_DIR}/target/jlink/codex/bin/build-runtime-artifacts" <<'BUILDER_EOF'
#!/usr/bin/env bash
set -euo pipefail
plugin_dir="$1"
client_dir="$2"
...
BUILDER_EOF
    chmod +x "${DISTRIBUTION_DIR}/target/jlink/codex/bin/build-runtime-artifacts"
}

run_build_script() {
    run env PATH="${STUB_BIN_DIR}:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
        bash "${DISTRIBUTION_DIR}/scripts/build-runtime-artifacts.sh"
}
...
@test "downloads verified git-filter-repo binary, invokes builder, and cleans generated source files" {
    ...
    [ "${status}" -eq 0 ]
    [ -f "${DISTRIBUTION_DIR}/target/runtime/builder-args.txt" ]
...
@test "fails fast on unsupported platform before invoking builder" {
    ...
    [ ! -e "${DISTRIBUTION_DIR}/target/runtime/builder-args.txt" ]
}
