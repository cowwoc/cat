...
""");
  ...
  public void loadInvokesSkillOutputForKnownLauncher() throws IOException
  {
    ...
    try (TestClaudeTool scope = new TestClaudeTool(tempPluginRoot, tempPluginRoot))
    {
      ...
      Files.writeString(hooksDir.resolve("test-output"), """
        #!/bin/bash
        java -m io.github.cowwoc.cat.client.claude/io.github.cowwoc.cat.client.test.TestSkillOutput "$@"
        """);
  ...
  public void loadPropagatesIoExceptionFromSkillOutput() throws IOException
  {
    ...
    try (TestClaudeTool scope = new TestClaudeTool(tempPluginRoot, tempPluginRoot))
    {
      ...
      Files.writeString(hooksDir.resolve("test-output"), """
        #!/bin/bash
        java -m io.github.cowwoc.cat.client.claude/io.github.cowwoc.cat.client.test.TestSkillOutputThrowsIo "$@"
        """);
  ...
  public void loadReturnsErrorStringForRuntimeException() throws IOException
  {
    ...
    try (TestClaudeTool scope = new TestClaudeTool(tempPluginRoot, tempPluginRoot))
    {
      ...
      Files.writeString(hooksDir.resolve("test-output"), """
        #!/bin/bash
        java -m io.github.cowwoc.cat.client.claude/io.github.cowwoc.cat.client.test.TestSkillOutputThrowsRuntime "$@"
        """);
  ...
  public void loadReturnsErrorStringForConstructorException() throws IOException
  {
    ...
    try (TestClaudeTool scope = new TestClaudeTool(tempPluginRoot, tempPluginRoot))
    {
      ...
      Files.writeString(hooksDir.resolve("test-output"), """
        #!/bin/bash
        M=io.github.cowwoc.cat.client.claude
        java -m $M/io.github.cowwoc.cat.client.test.TestSkillOutputThrowsFromConstructor "$@"
        """);
  ...
  public void loadReturnsClassNameForNullExceptionMessage() throws IOException
  {
    ...
    try (TestClaudeTool scope = new TestClaudeTool(tempPluginRoot, tempPluginRoot))
    {
      ...
      Files.writeString(hooksDir.resolve("test-output"), """
        #!/bin/bash
        M=io.github.cowwoc.cat.client.claude
        java -m $M/io.github.cowwoc.cat.client.test.TestSkillOutputThrowsNullMessage "$@"
        """);
  ...
  public void extractClassNameHandlesMultiLineLauncher()
  {
    String launcherContent = """
      ...
      exec "$DIR/java" \\
        -Xms16m -Xmx96m \\
        -XX:+UseSerialGC \\
        -m io.github.cowwoc.cat.client.claude/io.github.cowwoc.cat.claude.hook.skills.GetStatusOutput "$@"
      """;

    String className = GetSkill.extractClassName(launcherContent);
    requireThat(className, "className").isEqualTo("io.github.cowwoc.cat.claude.hook.skills.GetStatusOutput");
  }
  ...
  public void extractClassNameHandlesSingleLineLauncher()
  {
    String launcherContent = """
      #!/bin/sh
      exec java -m io.github.cowwoc.cat.client.claude/io.github.cowwoc.cat.claude.hook.UserPromptSubmitHook
      """;

    String className = GetSkill.extractClassName(launcherContent);
    requireThat(className, "className").isEqualTo("io.github.cowwoc.cat.claude.hook.UserPromptSubmitHook");
  }
...
""");
  ...
  public void loadInvokesSkillOutputWithClaudeToolConstructor() throws IOException
  {
    ...
    try (TestClaudeTool scope = new TestClaudeTool(tempPluginRoot, tempPluginRoot))
    {
      ...
      Files.writeString(hooksDir.resolve("test-output"), """
        #!/bin/bash
        java -m io.github.cowwoc.cat.client.claude/io.github.cowwoc.cat.client.test.TestClaudeToolSkillOutput "$@"
        """);
