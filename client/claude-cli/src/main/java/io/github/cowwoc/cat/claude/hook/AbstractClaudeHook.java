...
public abstract class AbstractClaudeHook extends AbstractClaudePluginScope implements ClaudeHook
{
  ...
  {
    super(projectPath, pluginRoot, pluginData, claudeConfigPath, workDir, timezone);
    requireThat(data, "data").isNotNull();
    this.data = data;
    this.sessionId = validateSessionId(data);
    this.agentId = validateAgentId(data);
  ...
  @Override
  public String getStringInput(String... keys)
  {
    ensureOpen();
    for (String key : keys)
    {
      String value = getStringInput(key);
      if (!value.isEmpty())
        return value;
    }
    return "";
  ...
  @Override
  public String getStringInput(String key, String defaultValue)
  {
    ensureOpen();
    String value = getStringInput(key);
    if (!value.isEmpty())
  ...
  @Override
  public boolean getBooleanInput(String key, boolean defaultValue)
  {
    ensureOpen();
    JsonNode node = data.get(key);
    if (node == null)
  ...
  @Override
  public JsonNode getObjectInput(String key)
  {
    ensureOpen();
    JsonNode node = data.get(key);
  ...
  {
    ensureOpen();
    return getStringInput("tool_name", "");
  }
  ...
  {
    ensureOpen();
    JsonNode node = getObjectInput("tool_input");
    if (node != null)
      return node;
  ...
  {
    ensureOpen();
    JsonNode node = getObjectInput("tool_result");
    if (node == null)
      node = getObjectInput("tool_response");
    if (node != null)
      return node;
    return getJsonMapper().createObjectNode();
  }
  ...
  {
    ensureOpen();
    return getStringInput("message", "user_message", "prompt");
  }
