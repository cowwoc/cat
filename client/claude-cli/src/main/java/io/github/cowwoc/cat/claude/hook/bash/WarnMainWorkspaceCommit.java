...
import io.github.cowwoc.cat.tool.util.WorktreeLock;
...
public final class WarnMainWorkspaceCommit implements BashHandler
{
   ...
   */
  public WarnMainWorkspaceCommit(ClaudeHook scope)
  {
  ...
  public Result check()
  {
    ...
    String workingDirectoryValue = scope.getStringInput("cwd");
    requireThat(workingDirectoryValue, "workingDirectory").isNotNull();
    String sessionId = scope.getSessionId();
    requireThat(sessionId, "sessionId").isNotBlank();
    Path workingDirectory = Path.of(workingDirectoryValue);
   ...
   */
  private static boolean isInsideCatWorktree(Path directory)
  {
    String gitDirPath;
    try
    {
      gitDirPath = GitCommands.runGit(directory, "rev-parse", "--git-dir");
    }
    catch (IOException _)
    {
    ...
    if (!gitDir.isAbsolute())
      gitDir = directory.resolve(gitDirPath).normalize();
   ...
   */
  private static Path extractCdDirectory(String command, Path fallback)
  {
    Matcher cdMatcher = CD_PATTERN.matcher(command);
    Path lastCdDir = fallback;
    while (cdMatcher.find())
    {
      ...
      if (!dir.isEmpty())
      {
        ...
        if (!resolved.isAbsolute())
          resolved = lastCdDir.resolve(dir).normalize();
        lastCdDir = resolved;
      }
    }
    return lastCdDir;
