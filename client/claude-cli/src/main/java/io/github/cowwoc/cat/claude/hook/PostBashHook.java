...
import static io.github.cowwoc.cat.tool.Strings.equalsIgnoreCase;
import static io.github.cowwoc.requirements13.java.DefaultJavaValidators.requireThat;

import io.github.cowwoc.cat.claude.hook.bash.post.DetectConcatenatedCommit;
import io.github.cowwoc.cat.claude.hook.bash.post.DetectFailures;
import io.github.cowwoc.cat.claude.hook.bash.post.ValidateRebaseTarget;
import io.github.cowwoc.cat.claude.hook.bash.post.VerifyCommitType;
...
public final class PostBashHook implements HookHandler
{
  ...
  /**
   * Creates a new PostBashHook instance with the specified JVM scope.
   *
   * @param scope the JVM scope providing access to shared resources
   * @throws NullPointerException if {@code scope} is null
   */
  public PostBashHook(ClaudeHook scope)
  {
    requireThat(scope, "scope").isNotNull();
    this.scope = scope;
    this.handlers = List.of(
      new DetectConcatenatedCommit(scope),
      new DetectFailures(scope),
      new ValidateRebaseTarget(scope),
      new VerifyCommitType(scope));
  }
  ...
  {
    HookRunner.execute(PostBashHook::new, scope, out);
  }
