...
public final class ValidateRebaseTarget implements BashHandler
{
   ...
   */
  public ValidateRebaseTarget(ClaudeHook scope)
  {
  ...
  public Result check()
  {
    ...
    if (!localCommit.equals(remoteCommit))
    {
      return Result.warn(String.format("""
        ...
        OPTION RANKING:
          1) git rebase %s (Rank #1)
             Reason: Default when user says "rebase on %s". Uses local branch state.
          2) git rebase origin/%s (Rank #2)
             Reason: Use only when user explicitly asks for remote tip.
