...
public final class BlockUnauthorizedMergeCleanupTest
{
  ...
  public void unrelatedCommandIsAllowed() throws IOException
  {
    ...
    try (TestClaudeHook scope = TestUtils.bashHook("git status", Path.of("/workspace"), SESSION_ID,
  ...
  public void highTrustAllowsWithoutApproval() throws IOException
  {
    ...
    try (TestClaudeHook scope = TestUtils.bashHook("/path/to/merge-and-cleanup session-id issue-id",
      Path.of("/workspace"), SESSION_ID, tempDir, tempDir, tempDir))
    {
  ...
  /**
   * Verifies that trust="medium" with plain chat approval blocks merge-and-cleanup.
   * <p>
   * The hook must require structured approval through AskUserQuestion.
   *
   * @throws IOException if test setup fails
   */
  @Test
  public void mediumTrustWithPlainChatApprovalBlocks() throws IOException
  {
    Path tempDir = Files.createTempDirectory("block-unauthorized-merge-cleanup-test-");
    try (TestClaudeHook scope = TestUtils.bashHook("/path/to/merge-and-cleanup session-id issue-id",
      Path.of("/workspace"), SESSION_ID, tempDir, tempDir, tempDir))
