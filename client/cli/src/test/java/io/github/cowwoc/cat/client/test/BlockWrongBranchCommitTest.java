...
public final class BlockWrongBranchCommitTest
{
  ...
  public void nonCommitCommandIsAllowed() throws IOException
  {
    ...
    {
      BashHandler.Result result = check(scope, "git status", tempDir);
  ...
  public void commitInWorktreeOnWrongBranchIsBlocked() throws IOException
  {
    ...
    try (TestClaudeTool scope = new TestClaudeTool(mainRepo, mainRepo))
    {
      ...
      try
      {
        ...
        BashHandler.Result result = check(scope, "git commit -m \"feature: wrong branch commit\"",
          wrongWorktreeDir);
  ...
  public void cdPathUsedForWorktreeDetection() throws IOException
  {
    ...
    try (TestClaudeTool scope = new TestClaudeTool(mainRepo, mainRepo))
    {
      ...
      try
      {
        ...
        BashHandler.Result result = check(scope, command, mainRepo);
  ...
  public void gitPushCommandIsAllowed() throws IOException
  {
    ...
    try (TestClaudeTool scope = new TestClaudeTool(mainRepo, mainRepo))
    {
      ...
      try
      {
        ...
        BashHandler.Result result = check(scope, "git push origin 2.1-push-test",
          worktreeDir);
  ...
  public void gitCommandFailureAllowsCommit() throws IOException
  {
    ...
    try (TestClaudeTool scope = new TestClaudeTool(tempDir, tempDir))
    {
      ...
      BashHandler.Result result = check(scope, "git commit -m \"feature: something\"",
        tempDir);
  ...
  public void cdWithRelativePathIsResolved() throws IOException
  {
    ...
    try (TestClaudeTool scope = new TestClaudeTool(mainRepo, mainRepo))
    {
      ...
      try
      {
        ...
        BashHandler.Result result = check(scope, command, siblingA);
  ...
  public void cdWithEmptyTargetIsIgnored() throws IOException
  {
    ...
    try (TestClaudeTool scope = new TestClaudeTool(mainRepo, mainRepo))
    {
      ...
      BashHandler.Result result = check(scope, "cd   && git commit -m \"feature: something\"",
        mainRepo);
  ...
  public void gitCommitVariantFlagsAreDetected() throws IOException
  {
    ...
    try (TestClaudeTool scope = new TestClaudeTool(mainRepo, mainRepo))
    {
      ...
      try
      {
        ...
        // Test: git commit -a
        BashHandler.Result result1 = check(scope, "git commit -a", worktreeDir);
        requireThat(result1.blocked(), "blockedForDashA").isTrue();

        // Test: git commit -am "message"
        BashHandler.Result result2 = check(scope, "git commit -am \"feature: add something\"",
          worktreeDir);
        requireThat(result2.blocked(), "blockedForDashAm").isTrue();

        // Test: git commit --message="message"
        BashHandler.Result result3 = check(scope, "git commit --message=\"feature: long form\"",
          worktreeDir);
  ...
  public void testIsolationBranchCommitIsAllowed() throws IOException
  {
    ...
    try (TestClaudeTool scope = new TestClaudeTool(mainRepo, mainRepo))
    {
      ...
      try
      {
        ...
        BashHandler.Result result = check(scope, "git commit -m \"test-runner workspace\"",
          worktreeDir);
        requireThat(result.blocked(), "blocked").isFalse();
      }
      finally
   ...
   */
  private static BashHandler.Result check(TestClaudeTool scope, String command,
    Path workingDirectory)
  {
    try (TestClaudeHook commandScope = TestUtils.bashHook(command, workingDirectory, SESSION_ID,
      scope))
    {
      return new BlockWrongBranchCommit(commandScope).check();
    }
  }
}
