...
public final class EnforceStatusOutput
{
  ...
  public static void run(ClaudeHook scope, String[] args, InputStream in, PrintStream out)
  {
    ...
    try
    {
      String transcriptPath = scope.getStringInput("transcript_path");
      boolean stopHookActive = scope.getBooleanInput("stop_hook_active", false);
    ...
    if (!sessionId.isEmpty())
    {
      ...
      if (Files.exists(flagPath))
      {
        String reason = """
          ...
          See skills/collect-results/SKILL.md in the installed CAT plugin for argument details.""";
