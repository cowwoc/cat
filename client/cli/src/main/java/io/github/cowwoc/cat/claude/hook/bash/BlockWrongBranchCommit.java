...
public final class BlockWrongBranchCommit implements BashHandler
{
   ...
   */
  public BlockWrongBranchCommit(ClaudeHook scope)
  {
  ...
  public Result check()
  {
    ...
    try
    {
      gitDirPath = GitCommands.runGit(effectiveDirectory, "rev-parse", "--git-dir");
    }
    catch (IOException _)
    {
    ...
    if (!gitDir.isAbsolute())
      gitDir = effectiveDirectory.resolve(gitDirPath).normalize();
    ...
    // During Step 6 of instruction-builder, the main agent creates a shared orphan branch named
   ...
   */
  private Path extractCdDirectory(String command, Path fallback)
  {
