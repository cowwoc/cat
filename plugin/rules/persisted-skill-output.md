...
(up to 400 lines at a time) to read the content in chunks — reading the entire file at once
...
3. When the persisted-output message states the file size (e.g., "30.3KB"), pre-compute the number of
   400-line chunks needed and issue ALL Read calls in a single parallel message — do NOT read one chunk
   at a time. Estimate total lines as `ceil(size_in_bytes / 45)`, then issue `ceil(total_lines / 400)`
   Read calls with `limit=400` each. Overshooting is fine — the Read tool returns only lines that exist.
   Example for a 30.3KB file (~674 lines, 2 chunks):
   - Read offset=1 limit=400
   - Read offset=401 limit=400
   Example for a 150KB file (~3334 lines, 9 chunks):
   - Read offset=1 limit=400
   - Read offset=401 limit=400
   - Read offset=801 limit=400
   - Read offset=1201 limit=400
   - Read offset=1601 limit=400
   - Read offset=2001 limit=400
   - Read offset=2401 limit=400
   - Read offset=2801 limit=400
   - Read offset=3201 limit=400
