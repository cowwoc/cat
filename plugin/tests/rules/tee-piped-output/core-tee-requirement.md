...
2. response must insert tee into the pipeline after the first command to capture output to LOG_FILE
3. response must use `tee "$LOG_FILE" |` pattern (tee feeding output to subsequent pipeline stage)
