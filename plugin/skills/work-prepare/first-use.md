...
**Diagnostic accuracy note:** The `blocked_tasks` diagnostic list is generated by a separate index scan
...
result is authoritative. If `blocked_tasks` shows `not_found` for a dependency, verify its actual status
