...
```
test_dir worktree_path test_model
```

Example invocation for a skill with 3 test cases expected to pass within 10–15 minutes:

```
/cat:sprt-runner  plugin/tests/skills/my-skill/  /path/to/worktree  sonnet
```
...
- `${CLAUDE_PLUGIN_DATA}/client/bin/instruction-test-runner` binary is available
- `${CLAUDE_PLUGIN_DATA}/client/bin/extract-turns` binary is available (splits multi-turn scenarios into individual turn files)
...
|---------|-------|----------|
| `run-full-sprt` | Complete SPRT workflow (public entry point) | Invoked by `sprt-runner` to run a full test suite. Orchestrates all 8 steps: prepare run, create isolation branch, initialize SPRT state, run batches via `run-sprt-batch`, write results, and cleanup. |
...
WORKTREE_PATH="$1"
TEST_MODEL="$2"
TEST_EFFORT="$3"  # Required: effort level (low/medium/high/xhigh/max)

# Resolve short model name to full model ID
TEST_MODEL_ID=$("${CLAUDE_PLUGIN_DATA}/client/bin/claude-runner" resolve-model "${TEST_MODEL}")
...
Bash tool:
  ...
  command: |
    "${CLAUDE_PLUGIN_DATA}/client/bin/instruction-test-runner" run-full-sprt \
...
5. Check if grade files and run artifacts still exist:
   ```bash
   TEST_RUNS_DIR="${WORKTREE_PATH}/.cat/work/test-runs/${CLAUDE_SESSION_ID}"
...
if grep -q "Pipeline for.*failed\|Grade file missing\|IOException\|ERROR" "${OUTPUT_FILE}"; then
  ...
  ls -la "${WORKTREE_PATH}/.cat/work/test-runs/${CLAUDE_SESSION_ID}/" 2>/dev/null || echo "Already cleaned up"
  echo ""
...
```bash
cd "${WORKTREE_PATH}/.cat/work/test-runs/${CLAUDE_SESSION_ID}"

# Find all grade files for failed test cases
for TC_DIR in tc*/ ; do
  TC_ID="${TC_DIR%/}"
  # Read the test case decision from sprt-state.json
  DECISION=$("${CLAUDE_PLUGIN_DATA}/client/bin/instruction-test-runner" get-json-field \
