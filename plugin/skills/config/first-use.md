...
```
FAIL: `<output skill="config">` not provided by the get-config-output preprocessor directive.
This output is generated by GetConfigOutput.java via the get-config-output launcher.
If this section is empty, the launcher or SkillOutput wiring may be missing.
...
Use the **CURRENT_SETTINGS** box from `<output skill="config">` (provided by get-config-output preprocessor).
...
Use the **CURRENT_SETTINGS** box from `<output skill="config">`.
...
Use the **SETTING_UPDATED** box from `<output skill="config">`.
...
Use the **CONFIGURATION_SAVED** box from `<output skill="config">`.
...
Use the **NO_CHANGES** box from `<output skill="config">`.
