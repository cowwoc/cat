...
```bash
"${CLAUDE_PLUGIN_DATA}/client/bin/get-stakeholder-selection-box" "${SELECTED_COUNT}" "10" "${SELECTED_LIST}" "${SKIPPED_LIST}"
```
...
```bash
"${CLAUDE_PLUGIN_DATA}/client/bin/get-stakeholder-review-box" "${ISSUE_ID}" "${REVIEWER_STATUS_LIST}" "${REVIEW_RESULT}" "${REVIEW_SUMMARY}"
```

For each concern, render concern box by invoking:

```bash
"${CLAUDE_PLUGIN_DATA}/client/bin/get-stakeholder-concern-box" "${SEVERITY}" "${STAKEHOLDER}" "${FILE_LOCATION}" "${CONCERN_DESCRIPTION}"
```
