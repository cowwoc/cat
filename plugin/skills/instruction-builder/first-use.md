...
[work merge phase](${CLAUDE_PLUGIN_ROOT}/skills/work-merge/first-use.md)
</execution_context>
...
Task tool:
  ...
  prompt: |
    ...
    Read and follow: ${CLAUDE_PLUGIN_ROOT}/skills/instruction-builder/skill-conventions.md
...
`${CLAUDE_PLUGIN_ROOT}/skills/instruction-builder/validation-protocol.md` (Section 1). Perform this
...
`plugin/skills/foo/first-use.md` must reference only paths under `plugin/skills/foo/` and
`plugin/tests/skills/foo/`.
...
args: "${TEST_DIR} ${WORKTREE_PATH} ${TEST_MODEL}"
```
...
```bash
# Extract overall_decision from sprt-runner output
...
if [[ "${OVERALL_DECISION}" != "ACCEPT" && "${OVERALL_DECISION}" != "REJECT" && "${OVERALL_DECISION}" != "INCONCLUSIVE" ]]; then
  echo "ERROR: sprt-runner returned invalid overall_decision: '${OVERALL_DECISION}'" >&2
  echo "Raw output:" >&2
  echo "${SKILL_OUTPUT}" >&2
  exit 1
fi
...
# Expected format: <subagent_id> <status> <description> — one subagent per line; $1 is the subagent ID field.
...
# subagent IDs are passed as raw agent IDs and resolved with current session context internally).
SANITIZED_IDS=""
...
Invoke cat:get-history with:
  ...
  args: "analyze --agent-id ${AGENT_ID}"
```
...
"$SESSION_ANALYZER" search "${CLAUDE_SESSION_ID}" \
  "Would you like|What would you|follow.up" --agent-id "${AGENT_ID}" --regex --context 5
```
...
"$SESSION_ANALYZER" search "${CLAUDE_SESSION_ID}" \
  "<thinking>" --agent-id "${AGENT_ID}" --context 10
```
...
"$SESSION_ANALYZER" search "${CLAUDE_SESSION_ID}" \
  "unless|except|if user|may|optional" --agent-id "${AGENT_ID}" --regex --context 3
```
...
`<directory-name>-<file-stem>` (e.g., `work-first-use` for
`plugin/skills/work/first-use.md`, `git-commit-SKILL` for
`plugin/skills/git-commit/SKILL.md`). This compound key avoids collisions when a single skill
...
Task tool:
  ...
  prompt: |
    ...
    Read and follow: ${CLAUDE_PLUGIN_ROOT}/skills/instruction-builder/compression-protocol.md
    ...
    (3) ${CLAUDE_PLUGIN_ROOT}/skills/instruction-builder/compression-protocol.md.
    ...
    NotebookEdit tool, or the Skill tool. Do NOT invoke any helper skill
    (e.g., cat:grep-and-read, or any other cat: skill).
    See ## Subagent Command Allowlist for permitted commands (grader/analyzer category applies here).
...
`${CLAUDE_PLUGIN_ROOT}/skills/instruction-builder/validation-protocol.md` (Section 2):
...
1. **Check if the Skill tool was invoked.** Use `session-analyzer file-history` on the subagent to
   ...
   "${CLAUDE_SESSION_ID}" "<thinking>" --agent-id "${ID}" --context 10`. The thinking blocks
...
- **Behavioral test cases**: SPRT calibration test cases for this skill are stored in
  `plugin/tests/skills/instruction-builder/first-use/` (one `.md` file per test case). Test results
  are stored in `plugin/tests/skills/instruction-builder/first-use/test-results.json`
