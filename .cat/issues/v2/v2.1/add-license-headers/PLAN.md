...
- **Skip:** `PLAN.md`, `STATE.md` files in `.cat/issues/` (planning artifacts)
- **Skip:** Files in `.cat/` directory (planning/config, not distributed source)

### Files to Skip
- `*.json` files (JSON does not support comments)
- `*.xml` files (configuration files, no semantic code)
- Files in `.cat/issues/` (planning artifacts)
- Files in `.cat/` config directory
- `LICENSE.md` itself
