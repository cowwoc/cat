...
- Session CWD file pattern: `.cat/sessions/{session_id}.cwd` (written after each Bash tool call)
...
- `client/src/main/java/io/github/cowwoc/cat/hooks/bash/TrackWorkingDirectory.java` (NEW) — PostToolUse Bash
  handler: writes `workingDirectory` from hook input to `.cat/sessions/{session_id}.cwd` when it is a
...
- `client/src/main/java/io/github/cowwoc/cat/hooks/session/RestoreCwdAfterCompaction.java` (NEW) — SessionStart
  handler: activates when `source` is `compact`; reads `.cat/sessions/{session_id}.cwd`; if path exists,
...
- `client/src/main/java/io/github/cowwoc/cat/hooks/session/SessionEndHandler.java` (or equivalent) — clean up
  `.cat/sessions/{session_id}.cwd` on session end
...
- [ ] After every Bash tool use with a non-project-root `workingDirectory`, `.cat/sessions/{session_id}.cwd`
