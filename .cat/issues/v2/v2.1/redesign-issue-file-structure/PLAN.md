...
structural relationship fields (dependencies, blocks, parent, decomposed_into) moved in alongside
status/resolution/target_branch. Remove the Progress field entirely. Remove Java code that parses
...
- [ ] `index.json` is valid JSON with fields: status, and conditionally resolution, target_branch,
  dependencies, blocks, parent, decomposed_into — no Progress field
...
1. Add new phases to `plugin/migrations/2.1.sh`:
   ...
   - Move dependencies, blocks, parent, decomposed_into from STATE.md key-value format into JSON
