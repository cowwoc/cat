...
- `.cat/cat-config.local.json` — remove from git tracking (git rm --cached)
- `.cat/issues/v2/v2.1/STATE.md` — add issue to Issues Pending list
...
3. **Step 3:** Run `git rm --cached .cat/cat-config.local.json` to remove the file from git tracking
...
## Post-conditions
- [ ] `git ls-files .cat/cat-config.local.json` returns empty (file is untracked)
- [ ] `.cat/cat-config.local.json` still exists on disk
