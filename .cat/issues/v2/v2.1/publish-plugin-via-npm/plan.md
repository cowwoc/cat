...
## Target State
- `marketplace.json` uses `"source": "npm"` pointing to the main npm package
- Main npm package declares `optionalDependencies` with `os`/`cpu` fields for platform-specific jlink bundles
- A manually-invoked CI workflow builds jlink bundles and publishes all npm packages
...
- **Concerns:**
  ...
  - npm publish requires auth token in CI secrets
- **Mitigation:**
  ...
  - Use `NPM_TOKEN` secret with `npm publish --provenance` for supply chain security

## Files to Modify

### npm package structure (new files)
- `plugin/package.json` — add `optionalDependencies` for platform sub-packages, configure `files` for npm publish
- `npm/cat-linux-x64/package.json` — platform sub-package (os: linux, cpu: x64)
- `npm/cat-linux-arm64/package.json` — platform sub-package (os: linux, cpu: arm64)
- `npm/cat-darwin-x64/package.json` — platform sub-package (os: darwin, cpu: x64)
- `npm/cat-darwin-arm64/package.json` — platform sub-package (os: darwin, cpu: arm64)
...
### Plugin source
- `.claude-plugin/marketplace.json` — change source from `"./plugin"` to `{ "source": "npm", "package": "claude-code-cat" }`
...
## Post-conditions
- [ ] `marketplace.json` references npm package as source
