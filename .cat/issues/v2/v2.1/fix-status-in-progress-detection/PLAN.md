...
- **Indie tier:** Check for lock files in `.cat/locks/`. Lock files are local to the machine, which matches the
...
1. **Add lock file detection:** In `getTaskStatus()`, after reading STATE.md, if status is `open`, check if a lock file
   exists at `.cat/locks/{version}-{issue-name}.lock`. If it does, return `in-progress`.
