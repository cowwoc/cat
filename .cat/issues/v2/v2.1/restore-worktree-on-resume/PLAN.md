...
- Lock files at `.cat/locks/{issue-name}.lock` contain `session_id` and `worktree` fields
- The `worktree` field in lock files is currently always empty - needs to be populated by `/cat:work`
- Worktree paths follow pattern: `.cat/worktrees/{issue-id}/`
...
1. `client/src/main/java/io/github/cowwoc/cat/hooks/session/RestoreWorktreeOnResume.java` (NEW)
   ...
   - Reads `session_id` from input, scans `.cat/locks/*.lock` for matching session
