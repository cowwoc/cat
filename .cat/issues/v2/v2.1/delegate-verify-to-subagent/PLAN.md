...
2. **Create verify subagent definition**
   ...
   - Agent writes detailed analysis to `<worktree>/.cat/verify/criteria-analysis.json`
   - Agent writes E2E test output to `<worktree>/.cat/verify/e2e-test-output.json`
     ...
     ```json
     {
       ...
       "criteria": [
         ...
         },
         {
           ...
           "detail_file": ".cat/verify/criteria-analysis.json"
         }
       ],
       "e2e": {
         "status": "PASSED",
         "explanation": "CLI tool produces expected output",
         "detail_file": ".cat/verify/e2e-test-output.json"
       }
     }
     ```
