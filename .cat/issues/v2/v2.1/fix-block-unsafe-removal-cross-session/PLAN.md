...
```json
{
  ...
  "worktree": "/workspace/.cat/worktrees/issue-name",
...
10. **E2E verification:** In the main workspace:
    - Create a lock with `agent_id`:
      ```bash
      echo '{"session_id":"aaaa","agent_id":"aaaa/subagents/xyz","created_at":1740700000,"worktree":"/workspace/.cat/worktrees/test-e2e","created_iso":"2026-02-28T00:00:00Z"}' > /workspace/.cat/locks/test-e2e.lock
      ```
    - Create dummy worktree: `mkdir -p /workspace/.cat/worktrees/test-e2e`
    - Test 1: `git worktree remove /workspace/.cat/worktrees/test-e2e` → blocked (no
      `CAT_AGENT_ID`)
