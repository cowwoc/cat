# Plan: Add .gitignore for .cat directory

## Goal
Create a standard `.gitignore` file in `.cat` that excludes temporary files (worktrees, locks, verification output) while preserving version control of issue definitions, configurations, and planning documents.
...
2. **Update /cat:init skill**
   - Add step to copy template to `.cat/.gitignore` if not exists
