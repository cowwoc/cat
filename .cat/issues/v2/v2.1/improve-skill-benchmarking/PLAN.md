...
4. **Persistent benchmark artifacts**: Record and commit benchmark data including skill/test case file paths, content
   hashes, and test case files themselves (stored in `test/` subdirectory under the skill directory).
...
- `plugin/skills/instruction-builder-agent/benchmark-runner.sh` - New script for incremental benchmarking
- `plugin/skills/instruction-builder-agent/test/` - Directory for test artifacts
...
- [ ] Test case files are stored in `test/` subdirectory under skill directory
...
1. **Step 1:** Implement diff logic to identify changed semantic units
   ...
   - Files: `plugin/skills/instruction-builder-agent/benchmark-runner.sh`
...
1. **Step 5:** Update to read skill `model:` field
   ...
   - Files: `plugin/skills/instruction-builder-agent/SKILL.md`, `plugin/skills/instruction-builder-agent/benchmark-runner.sh`
2. **Step 6:** Implement benchmark artifact persistence
   ...
   - Store test case files in `test/` subdirectory
...
1. **Step 7:** Implement token tracking in benchmark subagents
   ...
   - Files: `plugin/skills/instruction-builder-agent/benchmark-runner.sh`, hook files
