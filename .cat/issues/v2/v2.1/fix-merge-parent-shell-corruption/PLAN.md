...
## Root Cause
The parent agent spawns the merge subagent while its own shell cwd is inside the worktree (e.g., /workspace/.cat/worktrees/issue-name/). The subagent correctly cd's to /workspace before removing the worktree (its own shell is safe), but the parent shell remains in the deleted directory.
