...
- Rule files in `plugin/rules/` use the same YAML frontmatter format as `.cat/rules/` (audience filtering
...
- `InjectRulesToMainAgent` and `InjectRulesToSubAgent` read from TWO sources:
  ...
  2. `${projectDir}/.cat/rules/` — project-local rules (developer facing)
- Plugin-bundled rules are invisible in the end-user's `.cat/rules/` directory — they are read directly
...
- [ ] `InjectRulesToMainAgent` reads from both `${CLAUDE_PLUGIN_ROOT}/rules/` and `${projectDir}/.cat/rules/`
- [ ] `InjectRulesToSubAgent` reads from both sources
- [ ] Plugin-bundled rules are NOT visible in end-user's `.cat/rules/` directory
