...
### Current CAT state
`.cat/conventions/` holds on-demand coding standards (java.md, state-schema.md) that are loaded explicitly by
...
| `.cat/rules/` | CAT hooks | Content that needs audience filtering |

**Principle:** `.claude/rules/` is only for content that should reach every agent unconditionally. Everything else
lives in `.cat/rules/` where CAT controls who receives it.

### Frontmatter properties

Files in `.cat/rules/` support these optional frontmatter properties:
...
CAT hooks implement `paths` filtering independently of Claude Code's native support, since files in `.cat/rules/`
are not processed by Claude Code's native loader.

## Approach

### Phase 1: Audit content audience
Review all content in `.claude/rules/`, `CLAUDE.md`, and `.cat/conventions/` to classify each section:
- **All agents, unconditional**: Content that every agent always needs → stays in `.claude/rules/`
- **mainAgent-only**: Orchestration rules → `.cat/rules/` with `subAgents: []`
- **Both, conditional**: Coding conventions → `.cat/rules/` with `paths` frontmatter
- **Subagent-only**: Rules only for subagents → `.cat/rules/` with `mainAgent: false`

### Phase 2: Rename and reorganize
- Rename `.cat/conventions/` to `.cat/rules/`
...
- Move orchestration-specific content from `.claude/rules/` into `.cat/rules/` with `subAgents: []`
- Move coding conventions from `.cat/conventions/` into `.cat/rules/` with appropriate `paths` frontmatter
...
## Files to Modify
- `.cat/conventions/` — rename to `.cat/rules/`
- `.cat/conventions/INDEX.md` — update paths
- `.cat/conventions/java.md` — move to `.cat/rules/java.md`
- `.cat/conventions/state-schema.md` — move to `.cat/rules/state-schema.md`
...
- `.claude/rules/common.md` — extract non-universal sections into `.cat/rules/`
- `.claude/rules/hooks.md` — extract non-universal sections into `.cat/rules/`
...
3. **Implement frontmatter parsing**: Implement YAML frontmatter parser for `mainAgent`, `subAgents`, and `paths`
   properties in Java
4. **Update SessionStartHook**: Discover and inject `.cat/rules/` files filtered by `mainAgent: true` and
   `paths` matching
5. **Update SubagentStartHook**: Discover and inject `.cat/rules/` files filtered by matching `subAgents` type
   and `paths` matching
6. **Add audience frontmatter**: Add appropriate frontmatter to all files in `.cat/rules/`
7. **Move content**: Move non-universal sections from `.claude/rules/` into `.cat/rules/` with appropriate
...
9. **Write user-facing documentation**: Document the two-tier rules system in a user-facing doc (e.g.,
   `plugin/concepts/rules-audience.md`), explaining when to use `.claude/rules/` vs `.cat/rules/`, and the
...
## Post-conditions
- [ ] `.cat/conventions/` no longer exists; replaced by `.cat/rules/`
- [ ] `.claude/rules/` contains only content intended for all agents unconditionally
- [ ] All files in `.cat/rules/` have appropriate `mainAgent`/`subAgents` frontmatter
...
- [ ] All tests pass
- [ ] No references to `.cat/conventions/` remain in codebase
