...
# Call 1:
cd /workspace/.cat/worktrees/some-task
# Call 2 (separate Bash tool invocation):
git worktree remove --force /workspace/.cat/worktrees/some-task
# Hook sees System.getProperty("user.dir") = /workspace, not the shell's actual CWD
# /workspace is NOT inside /workspace/.cat/worktrees/some-task → allowed

# Bug 2: --force captured as path
echo "git worktree remove --force /workspace/.cat/worktrees/foo" | grep -oP 'git\s+worktree\s+remove\s+(\S+)'
