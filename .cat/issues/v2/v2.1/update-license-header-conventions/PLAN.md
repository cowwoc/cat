...
2. **Update exemptions in license-header.md**
   - Replace separate `.cat/issues/` and `.cat/` config entries with a single entry:
...
4. **Remove license headers from all exempt locations**
   - Scan `.cat/**` for files containing `Copyright (c) 2026` and remove the header block
...
- No files under `.cat/` have license headers
