...
`/workspace/.cat/issues/v2/v2.1/issue-name`. The `EnforceWorktreePathIsolation` hook blocks `Read` tool calls
...
```
# 1. Run /cat:work → work-prepare returns issue_path=/workspace/.cat/issues/v2/v2.1/issue-name
# 2. work-implement-agent sets: PLAN_MD="${ISSUE_PATH}/PLAN.md"
# 3. Agent invokes: Read("/workspace/.cat/issues/v2/v2.1/issue-name/PLAN.md")
# 4. EnforceWorktreePathIsolation hook blocks: "ERROR: Worktree isolation violation"
```

## Expected vs Actual

- **Expected:** Agents read PLAN.md from `${WORKTREE_PATH}/.cat/issues/v2/v2.1/issue-name/PLAN.md`
  (within worktree)
