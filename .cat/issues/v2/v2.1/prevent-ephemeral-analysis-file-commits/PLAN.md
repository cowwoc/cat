...
## Problem
`rebase-impact-agent` writes its analysis file to `${WORKTREE_PATH}/.cat/rebase-impact-analysis.md`,
...
- Update `plugin/skills/rebase-impact-agent/first-use.md`:
  ...
  - Update the `ANALYSIS_PATH` construction to use `session_analysis_dir` instead of `${WORKTREE_PATH}/.cat/`
...
## Post-conditions
- [ ] `rebase-impact-agent` `first-use.md` no longer references `${WORKTREE_PATH}/.cat/` as the output path
