...
2. **Directories not migrated** — The script removes `.gitignore` entries for these directories
   ...
   After migration, the following still exist under `.claude/cat/`:
...
- **Add Phase 10 — migrate cross-session directories:** Compute `{projectCatDir}` as
  ...
  `.claude/cat/locks/` exists, move its contents to `{projectCatDir}/locks/` and remove the source
  dir. Do the same for `.claude/cat/worktrees/`.
  - Files: `plugin/migrations/2.1.sh`

- **Add Phase 10 — delete session-scoped directories:** Unconditionally remove
  `.claude/cat/sessions/`, `.claude/cat/verify/`, and `.claude/cat/e2e-config-test/` when present
...
- **Write failing tests:** Add Bats tests that:
  ...
  - Verify `locks/` and `worktrees/` are absent from `.claude/cat/` after Phase 10
  - Verify `sessions/`, `verify/`, `e2e-config-test/` are absent from `.claude/cat/` after Phase 10
...
- [ ] After running `2.1.sh`, `.claude/cat/.gitignore` contains no orphaned comment lines (only
  `cat-config.local.json` remains, matching the template)
- [ ] After running `2.1.sh`, `.claude/cat/locks/` does not exist (contents moved to external storage
  or dir was already absent)
- [ ] After running `2.1.sh`, `.claude/cat/worktrees/` does not exist (contents moved to external
  storage or dir was already absent)
- [ ] After running `2.1.sh`, `.claude/cat/sessions/`, `.claude/cat/verify/`, and
  `.claude/cat/e2e-config-test/` do not exist
