...
- [ ] `get-output` entry exists in `client/build-jlink.sh`; individual output launchers removed (except
  `get-cleanup-output` which is retained as a separate launcher for its distinct stdin JSON interface used in
  plan/verify phases)
- [ ] `get-output/SKILL.md` calls launcher directly (not load-skill)
- [ ] All static-output skill `first-use.md` files use `!get-output X` pattern (GetOutput.java provides the
  `<output type="X">` wrapper automatically)
