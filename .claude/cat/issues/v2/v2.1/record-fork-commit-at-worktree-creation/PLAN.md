...
Change `cat-branch-point` from storing a branch name to storing the fork-point commit hash. All downstream git operations
...
`cat-branch-point` currently stores a branch name (e.g., `v2.1`). Every downstream skill that needs to compare against the fork
point must either:
...
- **Risk Level:** LOW
- **Concerns:** All worktree-aware code reads `cat-branch-point`; changing its format requires updating every reader
...
## Audit: `cat-branch-point` Consumers

### Existence-only checks (no change needed — work with any file content)

| File | Usage |
|------|-------|
| `EnforcePluginFileIsolation.java` | `Files.exists(catBranchPoint)` — detects worktree |
| `WarnBaseBranchEdit.java` | `Files.exists(catBranchPointFile)` — detects worktree |
| `VerifyStateInCommit.java` | `Files.exists(gitCatBase)` — detects worktree |
| `WarnUnsquashedApproval.java` | `Files.exists(catBranchPointFile)` — detects worktree |
...
| File | Pattern | Action |
|------|---------|--------|
| `stakeholder-review/first-use.md` | `git diff "${BASE_BRANCH}..HEAD"` | Read `cat-branch-point` as commit hash directly |
| `work-merge/first-use.md` | `git diff` with base branch | Read `cat-branch-point` as commit hash; pass branch name for merge |
| `work-with-issue/first-use.md` | `git diff` with base branch | Read `cat-branch-point` as commit hash |
| `git-squash/first-use.md` | `git rev-parse <base-branch>` | Read `cat-branch-point` directly (already a commit hash) |
| `cleanup/first-use.md` | `branch --merged` check | Read `cat-branch-point` as commit hash (works for `--merged`) |
| `learn/phase-prevent.md` | Metadata display | Display commit hash |

## Audit: Race-Condition Mitigations (Remove or Simplify)

With `cat-branch-point` storing an immutable fork-point commit, the following mitigations become redundant:
...
| Pin-once guidance | `git-squash/first-use.md:306-318` | "MANDATORY: Pin base branch reference before rebase" | `cat-branch-point` IS the pinned commit — no per-skill pinning needed |
| `git merge-base` calls in skills | Various skill files | Compute fork point at runtime | `cat-branch-point` provides the fork point directly |
| "Never Restore Files Using checkout" rule | `git-squash/first-use.md` | Warns against `git checkout <base-branch> -- <path>` | `cat-branch-point` is a commit hash, not a branch name — checkout with it is safe |
| "Use Merge-Base Diff" rule | `git-squash/first-use.md` | Warns against `git diff <base-branch>..HEAD` | `git diff $(cat cat-branch-point)..HEAD` is correct because `cat-branch-point` is the fork commit |
...
1. **Step 1:** Change `WorkPrepare.java` to write commit hash
   ...
   - Replace `Files.writeString(catBranchPointFile, baseBranch)` with `Files.writeString(catBranchPointFile, commitHash)` where
...
3. **Step 3:** Update `MergeAndCleanup` to accept base branch as CLI argument
   ...
   - Remove `getBaseBranch()` method that reads `cat-branch-point`
   - Use the passed branch name for `git fetch origin <branch>` and merge operations
   - Still read `cat-branch-point` for fork-point commit in diff/verification operations
   - Update tests
...
5. **Step 5:** Simplify skill files — remove redundant race-condition mitigations
   ...
   - Remove "Use Merge-Base Diff" section (diff with `cat-branch-point` commit hash is already correct)
   - Replace pin-once patterns (`BASE=$(git rev-parse $(cat cat-branch-point))`) with direct read (`BASE=$(cat cat-branch-point)`)
   - Remove redundant `git merge-base` computations

6. **Step 6:** Update worktree isolation tests
   - Files: `tests/worktree-isolation.bats`
   - Add test: `cat-branch-point` contains a commit hash, not a branch name
   - Add test: `cat-branch-point` value doesn't change when base branch advances
...
8. **Fix Step 8:** Fix `work-prepare/first-use.md` to write commit hash instead of branch name
   ...
   - Criterion: "All diff/verification operations use `cat-branch-point` directly as a commit hash"
   - At line 273 the script writes the branch name to `cat-branch-point`; replace with the output of `git rev-parse HEAD` so
     the fork-point commit hash is written instead
   - Verify no other location in the skill file writes a branch name to `cat-branch-point`

9. **Fix Step 9:** Remove `rev-parse` pin call from `GitRebaseSafe.java` Step 2
   - Files: `client/src/main/java/io/github/cowwoc/cat/hooks/util/GitRebaseSafe.java`
   - Criterion: "Per-skill pin-once patterns removed (`cat-branch-point` IS the pinned commit)"
   - Step 2 contains a `git rev-parse` call with a "Pin target ref to prevent race conditions" comment; remove this
     call and its comment because `cat-branch-point` already contains the immutable fork-point commit hash
...
11. **Fix Step 11:** Create `tests/worktree-isolation.bats` with `cat-branch-point` hash verification tests
    - Files: `tests/worktree-isolation.bats`
    - Criterion: "Bats tests verify `cat-branch-point` contains a commit hash and is immutable"
...
- [ ] `WorkPrepare` writes the fork-point commit hash to `cat-branch-point`
- [ ] All diff/verification operations use `cat-branch-point` directly as a commit hash
...
- [ ] Per-skill pin-once patterns removed (`cat-branch-point` IS the pinned commit)
- [ ] Race-condition documentation sections removed from `git-squash/first-use.md`
- [ ] All existing tests pass with no regressions
- [ ] Bats tests verify `cat-branch-point` contains a commit hash and is immutable
