...
Agents are primed to check lock status manually via bash commands (e.g., `ls /workspace/.claude/cat/locks/`) instead
...
`ls -la /workspace/.claude/cat/locks/` (directory does not exist), then `find` commands, before finally using
`issue-lock check`.
