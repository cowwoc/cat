...
2. **Duplicate `createTempProject()` methods** — Four files define near-identical methods that create a temp directory
   with `.claude/cat/` structure:
   - `IssueDiscoveryTest.java:1316` (creates `.claude/cat/issues/`)
   - `IssueLockTest.java:1013` (creates just a temp dir)
   - `IssueLockCliTest.java:374` (creates `.claude/cat/`)
   - `GetIssueCompleteOutputTest.java:714` (creates `.claude/cat/issues/`)
...
- `client/src/test/java/.../test/TestUtils.java` — Add `public static Path createTempCatProject(String prefix)`
  (creates `.claude/cat/issues/` structure)
...
excluded it because it only creates a plain temp directory (not `.claude/cat/issues/` structure). Its body is
